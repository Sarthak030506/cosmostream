...
exports.newsResolvers = {
    Query: {
         ...
         */
        async latestNews(_, { limit = 20, offset = 0 }, { user, db }) {
      ...
      `;
            ...
            // Get total count
            const countResult = await db.query("SELECT COUNT(*) as total FROM content_items WHERE content_type = 'news'");
         ...
         */
        async featuredNews(_, __, { user, db }) {
            // News is publicly accessible - no authentication required
            const query = `
        SELECT
          ci.id,
      ...
      `;
         ...
         */
        async newsByCategory(_, { categorySlug, limit = 20, offset = 0 }, { user, db }) {
            // News is publicly accessible - no authentication required
            // Get category ID
            const categoryResult = await db.query('SELECT id FROM content_categories WHERE slug = $1', [categorySlug]);
      ...
      `;
            ...
            // Get total count
            const countResult = await db.query("SELECT COUNT(*) as total FROM content_items WHERE content_type = 'news' AND category_id = $1", [categoryId]);
            const totalCount = parseInt(countResult.rows[0].total);
            const items = result.rows.map((row) => ({
                id: row.id,
                title: row.title,
         ...
         */
        async searchNews(_, { query, limit = 20, offset = 0 }, { user, db }) {
            // News is publicly accessible - no authentication required
            const searchQuery = `
        SELECT
          ci.id,
