...
exports.videoAnalyticsResolvers = void 0;
const graphql_1 = require("graphql");
const crypto_1 = __importDefault(require("crypto"));
exports.videoAnalyticsResolvers = {
    Query: {
        async videoAnalytics(_, { videoId, timeRange = 'LAST_30_DAYS' }, { db, user }) {
            // Check if video exists and user has permission to view analytics
            const videoResult = await db.query('SELECT creator_id FROM videos WHERE id = $1', [videoId]);
            ...
            if (user && (videoResult.rows[0].creator_id === user.id || user.role === 'admin')) {
                ...
                const summaryResult = await db.query(`SELECT * FROM get_video_analytics_summary($1, $2, CURRENT_DATE)`, [videoId, startDate.toISOString().split('T')[0]]);
            ...
            const viewsResult = await db.query('SELECT views FROM videos WHERE id = $1', [videoId]);
            return {
                videoId,
        ...
        async realtimeAnalytics(_, { videoId }, { db, user }) {
            // Check permission
            const videoResult = await db.query('SELECT creator_id FROM videos WHERE id = $1', [videoId]);
            if (videoResult.rows.length === 0) {
                throw new graphql_1.GraphQLError('Video not found', {
            ...
            const realtimeResult = await db.query('SELECT * FROM video_realtime_analytics WHERE video_id = $1', [videoId]);
            ...
            const currentViewersResult = await db.query(`SELECT COUNT(DISTINCT session_id) as current_viewers
         FROM video_views
         WHERE video_id = $1
           AND last_heartbeat_at >= NOW() - INTERVAL '5 minutes'
           AND completion_percentage < 95`, [videoId]);
            return {
                videoId,
    ...
    Mutation: {
        async trackVideoView(_, { input }, { db, user }) {
            try {
                // Hash IP address for privacy (if available from request context)
                const ipHash = crypto_1.default.createHash('sha256').update('anonymous').digest('hex');
                // Insert or update video view
                await db.query(`INSERT INTO video_views (
          ...
          DO UPDATE SET last_heartbeat_at = CURRENT_TIMESTAMP`, [
                ...
                await db.query('UPDATE videos SET views = views + 1 WHERE id = $1', [input.videoId]);
                return true;
            }
            catch (error) {
        ...
        async trackVideoEvent(_, { input }, { db, user }) {
            try {
                // Insert video event
                await db.query(`INSERT INTO video_events (
            video_id, session_id, user_id, event_type, video_timestamp, event_data
                ...
                if (input.eventType === 'session_end' || input.eventType === 'complete') {
                    ...
                    await db.query(`UPDATE video_views
             SET watch_duration = $1,
                 completion_percentage = $2,
                 completed_at = CASE WHEN $3 = 'complete' THEN CURRENT_TIMESTAMP ELSE completed_at END,
                 last_heartbeat_at = CURRENT_TIMESTAMP
             WHERE session_id = $4 AND video_id = $5`, [watchDuration, completionPercentage, input.eventType, input.sessionId, input.videoId]);
                }
                // Update heartbeat for active sessions
                if (['play', 'pause', 'seek'].includes(input.eventType)) {
                    await db.query(`UPDATE video_views
             SET last_heartbeat_at = CURRENT_TIMESTAMP
             WHERE session_id = $1 AND video_id = $2`, [input.sessionId, input.videoId]);
                }
                return true;
            }
            catch (error) {
    ...
    VideoAnalytics: {
        async retentionCurve(parent, _, { db }) {
            const result = await db.query(`SELECT timestamp_seconds, viewer_percentage, viewer_count, drop_off_count
         FROM video_retention
         WHERE video_id = $1
         ORDER BY timestamp_seconds ASC`, [parent.videoId]);
        ...
        async trafficSources(parent, _, { db }) {
            const result = await db.query(`SELECT traffic_source, COUNT(*) as view_count,
         ...
         ORDER BY view_count DESC`, [parent.videoId]);
        ...
        async deviceStats(parent, _, { db }) {
            const result = await db.query(`SELECT device_type, browser, operating_system, COUNT(*) as count
         FROM video_views
         WHERE video_id = $1
           AND started_at >= CURRENT_DATE - INTERVAL '30 days'
         GROUP BY device_type, browser, operating_system`, [parent.videoId]);
            const deviceTotals = {
                desktop: 0,
                mobile: 0,
                tablet: 0,
        ...
        async viewsByDate(parent, _, { db }) {
            const result = await db.query(`SELECT DATE(started_at) as date, COUNT(*) as value
         FROM video_views
         WHERE video_id = $1
           AND started_at >= CURRENT_DATE - INTERVAL '30 days'
         GROUP BY DATE(started_at)
         ORDER BY date ASC`, [parent.videoId]);
            return result.rows.map((row) => ({
                date: row.date,
                value: parseInt(row.value),
            }));
        },
        async topCountries(parent, _, { db }) {
            const result = await db.query(`SELECT country, COUNT(*) as view_count
         FROM video_views
         WHERE video_id = $1
...
//# sourceMappingURL=video-analytics.js.map