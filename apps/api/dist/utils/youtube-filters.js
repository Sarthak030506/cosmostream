...
exports.YouTubeContentFilter = void 0;
exports.initYouTubeContentFilter = initYouTubeContentFilter;
exports.getYouTubeContentFilter = getYouTubeContentFilter;
...
class YouTubeContentFilter {
     ...
     */
    checkVideoQuality(video, threshold) {
        const checks = [];
        // Check view count
        checks.push({
            name: 'view_count',
        ...
        const durationValid = video.durationSeconds >= MIN_VIDEO_DURATION &&
        ...
        if (threshold.content_rating === 'family_friendly') {
            const hasInappropriate = this.detectInappropriateContent(video.title + ' ' + video.description);
            checks.push({
     ...
     */
    async checkChannelQuality(channel, threshold) {
        const checks = [];
    ...
    filterVideos(videos, threshold) {
        ...
        for (const video of videos) {
            ...
            if (result.passed) {
                ...
                this.logger.debug(`Video "${video.title}" filtered out: ${result.reason} (score: ${result.score})`);
            }
        }
...
exports.YouTubeContentFilter = YouTubeContentFilter;
// Singleton instance
let filterInstance = null;
function initYouTubeContentFilter(logger) {
    if (!filterInstance) {
        filterInstance = new YouTubeContentFilter(logger);
    }
    return filterInstance;
}
function getYouTubeContentFilter() {
    if (!filterInstance) {
        throw new Error('YouTubeContentFilter not initialized');
    }
    return filterInstance;
}
exports.default = YouTubeContentFilter;
//# sourceMappingURL=youtube-filters.js.map