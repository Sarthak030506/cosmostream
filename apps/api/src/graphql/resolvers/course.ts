...
export const courseResolvers = {
  Query: {
    async course(_: any, { id }: any, { db }: Context) {
    ...
    async courses(_: any, { limit = 20, offset = 0 }: any, { db }: Context) {
      const result = await db.query(
        `SELECT id, title, description, creator_id, created_at,
  ...
  Mutation: {
    ...
    async addModuleToCourse(
      ...
      for (const videoId of videoIds || []) {
        await db.query(
          `INSERT INTO module_videos (module_id, video_id)
           VALUES ($1, $2)`,
          [module.id, videoId]
        );
      }
    ...
    async enrollInCourse(_: any, { courseId }: any, { user, db }: Context) {
      if (!user) {
        throw new GraphQLError('Not authenticated', {
          extensions: { code: 'UNAUTHENTICATED' },
        });
