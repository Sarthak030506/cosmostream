...
import { Pool } from 'pg';
import { getSpaceflightNewsService, NewsArticle } from './spaceflight-news';
...
class NewsSyncService {
  ...
  private spaceflightService: ReturnType<typeof getSpaceflightNewsService>;

  constructor(db: Pool, logger: winston.Logger) {
    this.db = db;
    this.logger = logger;
    this.spaceflightService = getSpaceflightNewsService();
  }

  /**
   * Sync latest news articles
   */
  async syncLatestNews(limit: number = 50): Promise<NewsSyncResult> {
    ...
    try {
      ...
      this.logger.info(
        `News sync completed: ${result.articlesImported} imported, ${result.articlesSkipped} skipped, ${result.articlesFailed} failed`
      );
   ...
   */
  async importNewsArticle(article: NewsArticle): Promise<boolean> {
    // Check if article already exists (by source URL)
    const existingArticle = await this.db.query(
      'SELECT id FROM content_items WHERE source_url = $1',
      [article.sourceUrl]
    );
   ...
   */
  private async getCategoryIdBySlug(slug: string): Promise<string> {
   ...
   */
  private async getSourceIdByName(name: string): Promise<string | null> {
   ...
   */
  private async updateSourceSyncTime(sourceName: string): Promise<void> {
    await this.db.query(
      'UPDATE news_sources SET last_synced_at = NOW(), updated_at = NOW() WHERE name = $1',
      [sourceName]
    );
  }
   ...
   */
  async deduplicateArticles(): Promise<number> {
  ...
  async getSyncStats(): Promise<any> {
    const totalArticles = await this.db.query(
      "SELECT COUNT(*) as count FROM content_items WHERE content_type = 'news'"
    );

    const lastSync = await this.db.query(
      'SELECT MAX(last_synced_at) as last_sync FROM news_sources WHERE sync_enabled = TRUE'
    );
...
// Singleton instance
let newsSyncService: NewsSyncService | null = null;

export function initNewsSyncService(db: Pool, logger: winston.Logger): NewsSyncService {
  if (!newsSyncService) {
